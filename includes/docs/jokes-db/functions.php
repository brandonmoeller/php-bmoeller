<?php
//
// This is the site-specific functions file for the docs/jokes-db site. 
//
// These are the site-specific variables:
$sitecss = "docs/jokes-db"; // Defines the site-specific stylesheet(s)
$sitejs = "docs/jokes-db"; // Defines the site-specific javascript(s)
...
function the_sitecss()
{
  ...
  if (strcasecmp($GLOBALS['sitecss'], "docs/jokes-db") == 0) {
    echo '<link rel="stylesheet" href="/css/docs/jokes-db/main.css?version=1.0">' . PHP_EOL;
  } else {
...
function the_sitejs()
{
  ...
  if (strcasecmp($GLOBALS['sitejs'], "docs/jokes-db") == 0) {
    include($_SERVER['DOCUMENT_ROOT'] . '/includes/docs/jokes-db/site-js.php');
  } else {
